...
- **Routine worker notifications are now optional.** New `showWorkerNotifications` boolean (default `true`) gates the observer/reflector/dropper progress toasts — `observer running on ~N-token chunk`, `N observations recorded`, `reflector running`, `dropper running`, and the info-level `no observations` notice. Set it to `false` (settings modal → UI → Worker notifications, `/blackhole configure`, the config file, or `PI_BLACKHOLE_SHOW_WORKER_NOTIFICATIONS`) for quiet sessions. Warnings and errors are unaffected: model fallback/unavailability, context-window skips, no-output warnings, worker failures, compaction notifications, and `/blackhole*` command output stay visible.
...
- **Memory workers end a finished run early and honor a prompt-cache preference.** The observer's `record_observations` and the reflector's `record_reflections` gained an optional `complete` flag: when the final batch is flagged `complete` and that batch rejected nothing, the host sends `terminate`, so a fully-processed run stops instead of spending the rest of its `agentMaxTurns` budget. The gate is per batch, so an incomplete batch — or a complete batch that rejected entries — keeps going and a later corrected batch can still close the run; the refusal is spelled out in the receipt so the model corrects instead of re-sending the same batch, and receipts report recorded/duplicate/rejected run totals. A model that omits the flag loses only the early stop — the batch still records. Closing with an empty `complete: true` batch is the single sanctioned "covered, nothing new" outcome and reports as info (`no_new_content`); both observer prompts instruct it, so a plain-text reply without a tool call stays a warning-level protocol slip. The reflector has no empty close (its batch is `minItems: 1`) and says so in the tool description. A new optional `cacheRetention` (`none` \| `short` \| `long`, unset = inherit pi's effective setting) is forwarded to all three worker streams so prompt caching can be tuned per install — case-insensitive on the file, env, and settings-modal paths, and adapters that do not support a value ignore it. `cacheRetention` is configurable in `/blackhole settings`, the config file, or `PI_BLACKHOLE_CACHE_RETENTION`; all three workers already scope their streams to the consolidation `sessionId`. The observer prompt keeps its append-stable prefix and never reintroduces local time ([upstream OM `#89`](https://github.com/elpapi42/pi-observational-memory/pull/89)).

### Removed

- **The dead `configure-overlay.ts` and `status-overlay.ts` TUI overlays are gone.** Neither had a production caller: `/blackhole configure` has routed to the pi-base settings modal (`/blackhole settings`) since the ConfigManager migration, and nothing ever opened the status overlay. Their test files go with them. User-facing behavior is unchanged — the settings modal is the only config UI, and `key-matcher.ts` (`visibleWidth`) stays for `/blackhole cleanup`.

### Fixed

- **Capped recall drill-downs tell the agent where to continue.** File/message drill-downs (`#N:path`, `#N:text`) returned their expansion uncapped, so a minified bundle or a verbose write dumped the whole payload into context, and the cap that did exist named a non-actionable `#N:path:offset:limit` placeholder. Every drill-down response is now bounded by `recallResponseMaxChars`, and `capDrillDownText()` appends a concrete continuation — `continue at #42:src/auth.ts:412:30` — naming the first line the cap did not show. The cut only ever lands on a line boundary, so following the hint neither skips nor repeats content; a line too big for the budget is named explicitly and paged past, and when no lines remain the footer points at a regex query instead. Paths containing colons round-trip through the parser unchanged.
...
- **`example-config.json` lists the whole config surface.** The file advertises `_all_settings_in_one_file` but had drifted: `showPreCompactionMessage`, `recallResponseMaxChars`, `compactAfterTokens`/`compactAfterRatio`/`compactReserveTokens`, `fullFoldAlways`, `statusBar`, `showWorkerNotifications`, and `dropperPoolFullnessThreshold` were all missing, so a reader had no reference for them beyond `docs/CONFIG.md`. Every key is now present with its DEFAULTS value, `_notes` and the env-override list cover the additions, and `cacheRetention` stays deliberately unset (documented in `_notes`) because pinning `short` downgrades a host whose effective cache setting is longer. A new `tests/example-config-completeness.test.ts` compares the file against `DEFAULTS`, so a future key cannot land without a reference here.
