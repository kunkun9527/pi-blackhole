...
describe("dedup algorithms", () => {
  ...
  describe("clusterObservations with drift guard", () => {
    it("preserves distinct facts even with legacy fuzzy options enabled", () => {
      ...
      expect(clusters.map((cluster) => cluster.rep.content)).toEqual(obs.map((item) => item.content));
      expect(clusters.map((cluster) => cluster.occurrences)).toEqual([1, 1, 1]);
    });
  });
});
