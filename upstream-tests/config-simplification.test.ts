...
describe("saveUnifiedConfig — atomic write", () => {
  ...
  it("returns false when the config destination is a directory", async () => {
    const { saveUnifiedConfig } = await import("../src/core/unified-config.js");
    // EISDIR is deterministic on Windows and Unix, including privileged users.
    // afterEach removes the fixture even if an assertion fails.
    mkdirSync(configPath(), { recursive: true });
    expect(saveUnifiedConfig({ compaction: "manual" })).toBe(false);
  });
});
