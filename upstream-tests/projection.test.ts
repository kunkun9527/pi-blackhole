...
describe("buildCompactionProjection", () => {
  ...
  it("triggers full fold when observations pool exceeds threshold", async () => {
    const { buildCompactionProjection } = await import("../src/om/ledger/projection.js");
    const obs = makeObservation(hexId("big-obs"), { content: "x".repeat(100_000), tokenCount: 0 });
...
describe('buildCompactionProjection — compact-all (firstKeptEntryId="")', () => {
  ...
  it("triggers full fold and caps observations when pool exceeds budget", async () => {
    ...
    // Live medium content exceeds the pool budget despite stale token counts.
    // The short high-relevance summary fits; the medium summary does not.
    const result = buildCompactionProjection(entries, "", {
...
describe("bounded compaction snapshots", () => {
  it.each([100, 0])(
    ...
    async (stored) => {
      ...
      // 18,900 live content tokens: stored metadata cannot suppress full folding.
      expect(result.fullFold).toBe(budget === 1000);
      expect(
  ...
  it.each([false, true])(
    ...
    async (fullFoldAlways) => {
      ...
      const fullFold = buildCompactionProjection(entries, "", {
        ...config,
        observationsPoolMaxTokens: 1,
      });
      expect(fullFold.fullFold).toBe(true);
      // No rendered observation fits one token; reflections have their own budget.
      expect(fullFold.observations).toEqual([]);
