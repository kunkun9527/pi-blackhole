...
describe("/blackhole-export", () => {
  ...
  it("exports tiers, dedup, reflections, dropper notes and orphans to markdown", async () => {
    ...
    // Distinct text is retained; only exact duplicates may collapse.
    expect(md.match(/markdown file to disk/g)?.length).toBe(1);
    expect(md.match(/markdown file too disk/g)?.length).toBe(1);
    expect(md).not.toContain("+1 variant");
